...
def state_to_key(state):
    """
    Converts the state dictionary into tuple of ints to allow Q-table indexing
    """
...
class TDControl_Agent():
    def __init__(self, 
        ...
        WE DONT KNOW THE SIZE (SHAPE) OF THE SYSTEM; IT IS TOO HIGH-DIMENSIONAL AND COMPLEX
        """        
        # the discount factor
        self.gamma = gamma
    ...
    def get_q_values(self, state):
        ...
        # Returns q-values for the given state, 0 if not visited
        return self.Qvalues.get(s_key, np.zeros(self.action_size))

    def get_best_action(self, s):
        """
        Chooses the best possible action from the Q-table (no exploration).
        """
        s_key = state_to_key(s)
        q_vals = self.Qvalues[s_key]
...
def evaluate_success_rate(agent, env, num_test_episodes=100, max_steps_per_episode=100):
    """
    Evaluates the agent in greedy mode (exploitation only), computing the success rate
    """
